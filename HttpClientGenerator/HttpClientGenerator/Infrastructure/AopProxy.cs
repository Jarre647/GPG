...
namespace HttpClientGenerator.Infrastructure
{
    ...
    public class AopProxy : DispatchProxy
    {
        ...
        /// </summary>
        public static T Create<T>(T decorated, string name, params MethodCallHandler[] handlers)
        {
            ...
            ((AopProxy) proxy).Initialize(name, decorated, handlers);
            return (T) proxy;
        }
        
        private void Initialize(string name, object decorated, MethodCallHandler[] handlers)
        {
            Name = name;
